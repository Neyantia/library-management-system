...
export default function BooksTable({ books, loadData }: BooksTableProps) {
    ...
    async function saveEdit(id: string) {
        ...
        const bookResponse = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/books/${id}`,
            {
                method: "PATCH",
                headers: {
                ...
                body: JSON.stringify({
                    title: editTitle,
                    authors: editAuthorsId ? [editAuthorsId] : [],
                    category: editCategoryId,
                    isbn: editISBN,
                    publicationYear: Number(editPublicationYear),
                    copiesCount: Number(editCopiesCount),
                    availableCopiesCount: Number(editAvailableCopiesCount)
                }),
            }
        );
                ...
                {books.map((book) => (
                    <tr key={book.id}>
                        ...
                        <td>
                            {editingId === book.id ? (
                                <input className="table-edit-input"
                                    value={editAvailableCopiesCount}
                                    onChange={(event) => setEditAvailableCopiesCount(event.target.value)}
                                />
                            ) : (
